 ...
 *
 * Usage: node scripts/index.js <slug> [--dry-run] [--threads-only] [--linkedin-only] [--force]
 *
 * 사전 조건:
 ...
 *   3. sns-publisher/.env 에 THREADS_*, LINKEDIN_* 토큰
 *
 * 경로 결정:
 *   - SNS_PUBLISHER_ROOT (=__dirname/..)   ← 본 패키지의 자체 폴더 (sns-publisher/)
 *   - CARDNEWS_ROOT (env)                  ← 카드뉴스가 빌드되는 외부 프로젝트 루트
 *                                             기본값: SNS_PUBLISHER_ROOT의 부모 폴더
 *   별도 repo로 분리할 때 CARDNEWS_ROOT 만 .env에 명시하면 됨.
 *
 * 산출:
...
const SNS_PUBLISHER_ROOT = path.resolve(__dirname, '..');

// Load .env from sns-publisher/.env (this package's own secrets).
try {
  require('dotenv').config({ path: path.join(SNS_PUBLISHER_ROOT, '.env') });
} catch {
  console.error('[fatal] dotenv missing. Run: cd sns-publisher && npm install');
  process.exit(2);
}

// CARDNEWS_ROOT: where output/ and themes/card-news/ live.
// Default = SNS_PUBLISHER_ROOT's parent folder (current MD-to-PPT layout).
const PROJECT_ROOT = process.env.CARDNEWS_ROOT
  ? path.resolve(process.env.CARDNEWS_ROOT)
  : path.resolve(SNS_PUBLISHER_ROOT, '..');
